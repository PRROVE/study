...
> 3. DPoS, Delegated Proof of Stake (위임 지분증명)  
>     * PoS에서 발전한 방식으로, **투표를 통해 대표 노드(검증자)를 선출**하여 블록 생성  
>     * 일반 참여자들이 직접 블록 생성에 참여하지 않고, **대표자에게 위임(delegation)**  
>     * 속도가 빠르고, 합의 과정이 간소화됨 `선출된 소수 노드만 블록 생성에 참여`  
>     * 중앙화 우려도 있음 `대표자 수가 적으면 권력 집중 가능성 존재`

> 4. PoA, Proof of Authority (권한증명)  
>     * **사전에 승인된 노드만 블록 생성 가능**한 방식  
>     * 신뢰할 수 있는 소수 노드에게 블록 생성 권한을 부여함 `신원 인증이 기반이 됨`  
>     * 속도와 효율이 매우 뛰어남 `기업, 프라이빗 블록체인에서 많이 사용`  
>     * 탈중앙화는 상대적으로 낮음 `관리자가 누구냐에 따라 시스템의 공정성이 달라질 수 있음`
